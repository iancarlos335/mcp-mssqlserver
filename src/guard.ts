...
export interface GuardViolation {
  keyword: string;
  // Perfil mínimo necessário; null = instrução desconhecida (allowlist estrita)
...
export function parseProfile(raw: string | undefined): Profile {
  ...
  throw new Error(
    `Valor inválido para MSSQL_PROFILE: ${raw}. Valores aceitos: reader, dml, ddl`
  );
}
 ...
 */
export function findViolation(query: string, profile: Profile): GuardViolation | null {
  if (profile === 'ddl') {
    return null;
  }
...
export function assertQueryAllowed(query: string, profile: Profile): void {
  ...
  const reason =
    violation.requiredProfile === null
  ...
  throw new Error(
    `Comando bloqueado pelo perfil de execução "${profile}": ${reason}\n${PROFILE_SUMMARY}`
  );
}

export function describeExecuteQueryTool(profile: Profile): string {
  switch (profile) {
    case 'reader':
      return (
        'Executa uma query SQL de leitura no SQL Server e retorna os resultados. ' +
        'Perfil ativo: "reader" — apenas SELECT e instruções de leitura são permitidos; ' +
        'INSERT/UPDATE/DELETE e DDL serão bloqueados.'
      );
    case 'dml':
      return (
        'Executa uma query SQL no SQL Server e retorna os resultados. ' +
